    ...
    function formatDataForMain() {
        ...
        data.forEach( (d, i) => {
            ...
            if (d.Platform.match(/^(PS|PS2|PS3|PS4|PSP|PSV)$/)) {
                game.console_company = "sony"; games[0].push(game);
            } else if (d.Platform.match(/^(XB|X360|XOne)$/)) {
                game.console_company = "microsoft"; games[1].push(game);
            } else if (d.Platform.match(/^(3DS|DS|GB|GBA|GC|N64|NES|SNES|Wii|WiiU)$/)) {
                game.console_company = "nintendo"; games[2].push(game);
            } else if (d.Platform == "PC") {
                game.console_company = "pc"; games[3].push(game);
            } else {
                game.console_company = "others"; games[4].push(game);
            }
        });
        return games;
    }
